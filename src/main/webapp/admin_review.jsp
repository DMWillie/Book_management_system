...
<%@ page import = "dao.AdminDao,dao.ReviewDao,bean.BookBean,dao.BookDao,bean.AdminBean,bean.ReviewBean" %>
...
<button type="button" class="btn btn-warning btn-xs" onclick="NO(<%= bean.getRid() %>)">不同意</button>
    ...
    function YES(rid){
    	...
    	if(con==true){			//status=1代表同意
    		location.href = "/books/ReviewServlet?status=1&rid="+rid;
		}
	}
    ...
    function NO(rid) {
    	...
    	if(con==true){			//result=2代表不同意
    		location.href = "/books/ReviewServlet?status=2&rid="+rid;
    	}
    }
    </script>
<!-------------------------------------------------------------->  
